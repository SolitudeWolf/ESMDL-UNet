...
def dice_coefficient(preds, gts, threshold=0.5):
    preds = np.where(preds >= threshold, 1, 0)
    gts = np.where(gts >= threshold, 1, 0)
    intersection = np.sum(preds * gts)
    union = np.sum(preds) + np.sum(gts)
    if union == 0:
        return 1.0
    return 2.0 * intersection / union

def hausdorff_distance(preds, gts, threshold=0.5):
    preds = np.where(preds >= threshold, 1, 0)
    gts = np.where(gts >= threshold, 1, 0)
    if np.sum(preds) == 0 or np.sum(gts) == 0:
        return np.inf
    d1 = directed_hausdorff(preds, gts)[0]
    d2 = directed_hausdorff(gts, preds)[0]
    return max(d1, d2)
def set_seed(seed):
    # for hash
...
def get_optimizer(config, model):
    ...
    elif config.opt == 'Lion':  # Add this condition for Lion optimizer
        return Lion(
            model.parameters(),
            lr=config.lr,
            betas=config.betas,
