...
def register_model(fn):
    ...
    if hasattr(mod, 'default_cfgs') and model_name in mod.default_cfgs:
        # this will catch all modules that have entrypoint matching cfg key, but miss any aliasing
...
def list_models(filter='', module='', pretrained=False, exclude_filters='', name_matches_cfg=False):
    ...
    Args:
        ...
        pretrained (bool) - Include only modules with pretrained weights if True
        exclude_filters (str or list[str]) - Wildcard filters to exclude modules after including them with filter
        name_matches_cfg (bool) - Include only modules w/ model_name matching default_cfg name (excludes some aliases)

    Example:
        model_list('gluon_resnet*') -- returns all modules starting with 'gluon_resnet'
        model_list('*resnext*, 'resnet') -- returns all modules with 'resnext' in 'resnet' module
    """
    if module:
    ...
    if filter:
        ...
        for f in include_filters:
            include_models = fnmatch.filter(all_models, f)  # include these modules
    ...
    if exclude_filters:
        ...
        for xf in exclude_filters:
            exclude_models = fnmatch.filter(models, xf)  # exclude these modules
...
def list_modules():
    """ Return list of module names that contain modules / model entrypoints
    """
