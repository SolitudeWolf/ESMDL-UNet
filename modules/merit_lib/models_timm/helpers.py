...
def remap_checkpoint(model, state_dict, allow_reshape=True):
    ...
    This assumes modules (and originating state dict) were created with params registered in same order.
    """
    out_dict = {}
...
def load_custom_pretrained(
    ...
    if load_from == 'hf-hub':  # FIXME
        _logger.warning("Hugging Face hub not currently supported for custom load pretrained modules.")
    elif load_from == 'url':
...
def checkpoint_seq(
    ...
    r"""A helper function for checkpointing sequential modules.

    Sequential modules execute a list of modules/functions in order
