...
There are multiple sets of modules defined for both architectures. Typically, names with a
...
The modules without extra prefix / suffix' (coatnet_0_224, maxvit_tiny_224, etc), are intended to
match paper, BUT, without any official pretrained weights it's difficult to confirm a 100% match.

# FIXME / WARNING
This impl remains a WIP, some configs and modules may vanish or change...
...
def _rw_coat_cfg(
    ...
    # 'RW' timm variant modules were created and trained before seeing https://github.com/google-research/maxvit
    # Common differences for initial timm modules:
    ...
    # Variable differences (evolved over training initial modules):
...
def _rw_max_cfg(
    ...
    # 'RW' timm variant modules were created and trained before seeing https://github.com/google-research/maxvit
    # Differences of initial timm modules:
    # - mbconv expansion calculated from input instead of output chs
    # - mbconv shortcut and final 1x1 conv did not have a bias
    # - mbconv uses silu in timm, not gelu
    # - expansion in attention block done via output proj, not input proj
...
def _next_cfg(
    ...
    # For experimental modules with convnext instead of mbconv
