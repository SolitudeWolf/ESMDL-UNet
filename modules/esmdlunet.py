...
import math
from .mewunet import MEWB
# 导入swinT
# 验证SwinT模块的功能，可以创建一些测试数据
# test_data = paddle.ones([2, 96, 224, 224]) # [N, C, H, W]
from .SwinTransformermain.models.swin_transformer import SwinTransformerBlock as SwinT


class NewDepthWiseConv2d(nn.Module):
...
class ESMDLUNet(nn.Module):

    def __init__(self, num_classes=1, input_channels=3, c_list=[8, 16, 24, 32, 48, 64], depth=[1, 2, 2, 4],mlp_ratio=4,bridge=True, gt_ds=True,
                 patch_size=2):
        super().__init__()
        ...
        self.patch_embed_2 = nn.Conv2d(c_list[0], c_list[1], 3, stride=self.patch_size, padding=1)
        self.patch_embed_3 = nn.Conv2d(c_list[1], c_list[2], 3, stride=self.patch_size, padding=1)
        self.patch_debed_4 = nn.Conv2d(c_list[2], c_list[1], 3, stride=1, padding=1)
        self.patch_debed_5 = nn.Conv2d(c_list[1], c_list[0], 3, stride=1, padding=1)
        ...
        # Assuming that SwinTransformerBlock maintains spatial dimensions
        self.encoder2 = nn.Sequential(
            NewDepthWiseConv2d(c_list[0], c_list[1]),
            MEWB(c_list[1], depth[0], mlp_ratio)
        )

        self.encoder3 = nn.Sequential(
            SwinT(
            dim=c_list[1],
            input_resolution=(self.input_resolution[0] // 4, self.input_resolution[1] // 4),
            window_size=8,
            num_heads=8,)
        )
        # self.encoder2 = nn.Sequential(
        #     KanConv(c_list[0], c_list[1], 3, stride=1, padding=1),
        # )
        # self.encoder3 = nn.Sequential(
        #     KanConv(c_list[1], c_list[2], 3, stride=1, padding=1),
        # )
        ...
        self.decoder4 = nn.Sequential(
            SwinT(
                dim=c_list[2],
                input_resolution=(self.input_resolution[0] // 8, self.input_resolution[1] // 8),
                window_size=8,
                num_heads=8, )
        )
        self.decoder5 = nn.Sequential(
            MEWB(c_list[1], depth[0], mlp_ratio),
            NewDepthWiseConv2d(c_list[1], c_list[0])
        )
...
#         v2 = v2.view(b, -1, c)
#         # v2 = F.gelu(F.max_pool2d(self.ebn2(self.SwinT2(v2)), 2, 2))
#         v2 = self.encoder2(v2)
#         v2_cnn = v2.view(b, h, w, c)
#         v2_cnn = v2_cnn.permute(0, 3, 1, 2).contiguous()
#         v2_cnn=self.patch_embed_2(v2_cnn)
#         v2_cnn=F.gelu(v2_cnn)
#         t2 = v2_cnn
        ...
        v3 = v3.view(b, -1, c)
        v3 = self.encoder3(v3)
        v3_cnn = v3.view(b, h, w, c)
        v3_cnn = v3_cnn.permute(0, 3, 1, 2).contiguous()
        v3_cnn=self.patch_embed_3(v3_cnn)
        v3_cnn=F.gelu(v3_cnn)
        t3 = v3_cnn

        out = F.gelu(F.max_pool2d(self.ebn4(self.encoder4(v3_cnn)), 2, 2))
        ...
        if self.gt_ds:
            ...
            t4 = self.GAB4(t5, t4)
        out4 = torch.add(out4, t4)  # b, c3, H/16, W/16  b 32 16 16

        out3 = F.gelu(F.interpolate(self.dbn3(self.decoder3(out4)), scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))  # b, c2, H/8, W/8  b 24 32 32
        if self.gt_ds:
        ...
        out3 = out3.view(b, -1, c)
        out3 = self.decoder4(out3)
        out3 = out3.view(b, h, w, c)
        out3 = out3.permute(0, 3, 1, 2).contiguous()
        out3 = self.patch_debed_4(out3)
        out2 = F.gelu(F.interpolate(out3, scale_factor=(2, 2), mode='bilinear',
                                    align_corners=True))
        # out3=self.decoder4(out3)
        # # print("out3.shape:", out3.shape) # 18, 24, 32, 32
        # out3=self.patch_debed_4(out3)
        # # print("out3.shape:", out3.shape) # 18, 16, 16, 16
        # out2 = F.gelu(F.interpolate(out3))  # b, c1, H/4, W/4  b 16 64 64
        ...
        # out2 = self.patch_debed_5(out2)
        out1 = F.gelu(F.interpolate(self.decoder5(out2), scale_factor=(2, 2), mode='bilinear',
